calendarBackEndApp.controller('updateCampaignController', function ($scope, calendarBackEndModel, getJson,$route, $routeParams, $location, $log, $q, $timeout) {
	...
	$scope.updateNewEvent = function () {
		...
		if (validarFormulario(formulario)) {
			...
			calendarBackEndModel.updateEvent(formulario).then(function (result) {
				var data = result.d;
				try {
					var dataJson = $.parseJSON(data);
					if (dataJson.status === "ok") {
						...
						$scope.loadingHide();
						$scope.message = {
							content: "Campaign Calendar Updated...",
							title: "Attention"
						}
