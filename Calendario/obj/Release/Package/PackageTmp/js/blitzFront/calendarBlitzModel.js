...
calendarBlitzApp.service('calendarBlitzModel', function ($http, $q) {
    ...
    function getEventsByMonthYear(month,year) {
        var request = $http({
            method: 'POST', url: "ServiciosWeb/CalendarService.asmx/GetEventsBlitzByMonthYear", contentType: 'application/json; charset=utf-8', data: { 'month': month,'year':year }
        });
        return (request.then(handleSuccess, handleError));
    }

    function searchServerV2(fecha, teatrosSeleccionados, campBlitzPartnerName, campBlitzStatus, campBlitzName,agency) {
        var request = $http({
            method: 'POST',
            url: "ServiciosWeb/CalendarService.asmx/SearchServerBlitzV2",
            contentType: 'application/json; charset=utf-8',
            data: { 'fecha': fecha, 'teatrosSeleccionados': teatrosSeleccionados, 'campBlitzPartnerName': campBlitzPartnerName, 'campBlitzStatus': campBlitzStatus, 'campBlitzName': campBlitzName, 'agency': agency }
        });
        return (request.then(handleSuccess, handleError));
    }
    ...
    function getEventsActualDate() {
        var request = $http({
            method: 'POST', url: "ServiciosWeb/CalendarService.asmx/GetEventsBlitzActualDate", contentType: 'application/json; charset=utf-8', data: { 'vista': 'vista' }
        });
        return (request.then(handleSuccess, handleError));
    }
