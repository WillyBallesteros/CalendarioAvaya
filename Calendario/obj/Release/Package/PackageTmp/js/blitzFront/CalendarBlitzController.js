...
calendarBlitzApp.controller('calendarBlitzController', function ($scope, calendarBlitzModel, getJson, $route, $routeParams, $location, $log, $q, $timeout) {
	...
	$scope.getEventsByMonthYear = function (date) {
		...
		if ($scope.existeBusqueda()) {
			...
			var campBlitzPartnerName = ($scope.CampBlitzPartnerName !== undefined) ? $scope.CampBlitzPartnerName : "";
			var campBlitzStatus = ($scope.CampBlitzStatus !== undefined) ? $scope.CampBlitzStatus : "";
			...
			$scope.searchServer(date, teatrosSeleccionados, campBlitzPartnerName, campBlitzStatus, campBlitzName,agency);
	...
	/**  INICIO DEL FILTRO  **/
	$scope.searchServer = function (actualDate, teatrosSeleccionados, campBlitzPartnerName, campBlitzStatus, campBlitzName,agency) {
		$scope.loadingShow();
		$scope.semana1 = {};
		$scope.semana2 = {};
		$scope.semana3 = {};
		$scope.semana4 = {};
		$scope.semana5 = {};
		calendarBlitzModel.searchServerV2(actualDate, teatrosSeleccionados, campBlitzPartnerName, campBlitzStatus, campBlitzName,agency).then(function (result) {
			var data = result.d;
			try {
	...
	$scope.existeBusqueda = function () {
		...
		if (campBlitzName || campBlitzPartnerName || campBlitzStatus || teatrosSeleccionados || agency) {
			return true;
		}
		return false;
	};
	...
	$scope.search = function () {
		...
		var actualDate = new Date(semana3[0].Date);
		$scope.searchServer(actualDate, teatrosSeleccionados, campBlitzPartnerName, campBlitzStatus, campBlitzName,agency);
